...
export const SearchResultItem: React.FC<SearchResultItemProps> = ({ item, isSelected, onMouseDown }) => {
  ...
  const parentInfo = ((): ParentInfo | null => {
    ...
    if (display) {
      if (display.icon.type === 'area') {
        return { icon: <AreaIcon className={desktopStyles.CommandPaletteResultItemParentAreaIcon} />, title: display.title };
      }
      return {
    ...
    if (item.type === 'task') {
      return {
        icon: <InboxIcon className={desktopStyles.CommandPaletteResultItemParentInboxIcon} />,
  ...
  const renderIcon = () => {
    switch (item.type) {
      case 'area':
        return <AreaIcon className={desktopStyles.CommandPaletteResultItemEntityAreaIcon} />;
